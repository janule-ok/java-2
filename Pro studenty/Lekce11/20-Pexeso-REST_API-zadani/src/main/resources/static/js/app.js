...
function priKliknutiNaKartu(evt) {
    ...
    var regex = /karticka_(\d+)/;
    var match = regex.exec(element.id);
    var poziceKarty = match[1];
    odesliTahNaServer(poziceKarty);
}

function odesliTahNaServer(poziceKarty) {
    odesliHttpRequest("POST", zpracujVyslednouHerniPlochu, "{\"poziceKarty\":" + poziceKarty + "}");
}
